...
import { FaGithub, FaStar, FaCodeBranch, FaUsers, FaFolderOpen, FaExternalLinkAlt, FaBookOpen } from "react-icons/fa";
...
const GithubStats: React.FC = () => {
  ...
  useEffect(() => {
    const fetchGitHubData = async () => {
      try {
        ...
        const reposRes = await fetch("https://api.github.com/users/Ebikemeese/repos?sort=updated&per_page=6");
        if (reposRes.ok) {
  ...
  return (
    <div id="github-stats" className="bg-[#1b1c2b] px-4 py-16 sm:py-20 relative border-t border-purple/10 overflow-hidden">
      <div className="w-full max-w-7xl mx-auto space-y-12">
        ...
        <div className="text-center">
          ...
          <p className="mt-3 text-secondary text-sm sm:text-base max-w-2xl mx-auto px-2">
            Real-time GitHub metrics dynamically fetched from my public GitHub profile.
          </p>
        </div>
        ...
        {loading ? (
          <div className="flex justify-center items-center p-8 bg-[#151030] rounded-2xl animate-pulse">
            <div className="text-purple font-semibold text-base sm:text-lg">Fetching live GitHub profile...</div>
          </div>
        ) : userData ? (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center bg-[#151030] p-6 sm:p-8 rounded-3xl border border-purple/20 shadow-2xl">
            {/* Left Profile Details */}
            <div className="lg:col-span-5 flex flex-col sm:flex-row lg:flex-col items-center sm:items-start lg:items-center text-center sm:text-left lg:text-center border-b lg:border-b-0 lg:border-r border-purple/20 pb-6 lg:pb-0 lg:pr-6 gap-4 sm:gap-6">
              {!failedImages["avatar"] ? (
                <img
                  src={userData.avatar_url}
                  alt={userData.name || userData.login}
                  className="w-24 h-24 sm:w-28 sm:h-28 rounded-full border-2 border-purple shadow-xl object-cover"
                  onError={() => handleImageError("avatar")}
                />
              ) : (
                <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-full border-2 border-purple bg-purple/20 flex items-center justify-center text-purple text-4xl shadow-xl">
                  <FaGithub />
                </div>
              )}

              <div>
                <h3 className="text-white text-xl sm:text-2xl font-bold">{userData.name || userData.login}</h3>
                <a
                  href={userData.html_url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-purple text-sm hover:underline inline-flex items-center gap-1 mt-1 font-semibold"
                >
                  @{userData.login} <FaExternalLinkAlt className="text-xs" />
                </a>
                <p className="text-gray-300 text-xs sm:text-sm mt-2 leading-relaxed max-w-md">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="mt-4 inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-semibold text-xs rounded-xl shadow-md transition-all"
                >
            ...
            <div className="lg:col-span-7 grid grid-cols-2 sm:grid-cols-3 gap-4">
              <div className="bg-[#252838] p-4 rounded-2xl border border-purple/10 flex flex-col justify-between shadow-md">
                <div className="text-purple text-2xl"><FaFolderOpen /></div>
                <div className="mt-4">
                  <div className="text-2xl sm:text-3xl font-bold text-white">{userData.public_repos}</div>
                  <div className="text-xs text-gray-400 font-medium mt-1">Public Repositories</div>
                </div>
              </div>
              ...
              <div className="bg-[#252838] p-4 rounded-2xl border border-purple/10 flex flex-col justify-between shadow-md">
                ...
                <div className="mt-4">
                  <div className="text-2xl sm:text-3xl font-bold text-white">{userData.followers}</div>
                  <div className="text-xs text-gray-400 font-medium mt-1">Followers</div>
                </div>
              </div>

              <div className="bg-[#252838] p-4 rounded-2xl border border-purple/10 flex flex-col justify-between shadow-md col-span-2 sm:col-span-1">
                <div className="text-yellow-400 text-2xl"><FaStar /></div>
                <div className="mt-4">
                  <div className="text-2xl sm:text-3xl font-bold text-white">
                    {repos.reduce((acc, repo) => acc + repo.stargazers_count, 0)}
                  </div>
                  <div className="text-xs text-gray-400 font-medium mt-1">Recent Stars</div>
                </div>
              </div>
            </div>
          </div>
        ) : error ? (
          <div className="text-center text-red-400 text-sm p-4 bg-[#151030] rounded-xl">{error}</div>
        ) : null}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {!failedImages["readme-stats"] && (
              <div className="bg-[#151030] p-4 sm:p-6 rounded-3xl border border-purple/20 flex flex-col items-center justify-center shadow-xl overflow-hidden">
                <h4 className="text-white text-sm font-semibold mb-4 text-center">GitHub Readme Stats</h4>
                <img
            ...
            {!failedImages["streak-stats"] && (
              <div className="bg-[#151030] p-4 sm:p-6 rounded-3xl border border-purple/20 flex flex-col items-center justify-center shadow-xl overflow-hidden">
                <h4 className="text-white text-sm font-semibold mb-4 text-center">GitHub Streak Stats</h4>
                <img
        ...
        {(!failedImages["top-langs"] || !failedImages["activity-graph"]) && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            {!failedImages["top-langs"] && (
              <div className={`bg-[#151030] p-4 sm:p-6 rounded-3xl border border-purple/20 flex flex-col items-center justify-center shadow-xl overflow-hidden ${
                failedImages["activity-graph"] ? "lg:col-span-12" : "lg:col-span-5"
              }`}>
                <h4 className="text-white text-sm font-semibold mb-4 text-center">Most Used Languages</h4>
                <img
            ...
            {!failedImages["activity-graph"] && (
              <div className={`bg-[#151030] p-4 sm:p-6 rounded-3xl border border-purple/20 flex flex-col items-center justify-center shadow-xl overflow-hidden ${
                ...
                <h4 className="text-white text-sm font-semibold mb-4 text-center">GitHub Contribution Graph</h4>
                <img
                  ...
                  <div className="mt-6 flex justify-between items-center text-xs text-gray-400 border-t border-purple/10 pt-3">
                    ...
                    <div className="flex items-center gap-3">
                      <span className="flex items-center gap-1">
                        <FaStar className="text-yellow-400 text-xs" /> {repo.stargazers_count}
                      </span>
                      <span className="flex items-center gap-1">
                        <FaCodeBranch className="text-cyan-400 text-xs" /> {repo.forks_count}
                      </span>
                    </div>
                  </div>
                </motion.div>
